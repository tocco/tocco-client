...
import {getAllDirectories, packagesExists, readFile} from '../lib/packages'
...
const copyPackage = () => {
  ...
  const packageDir = paths.client(`packages/${packageName}`)
  ...
  exec(
    ...
    (error, stdout, stderr) => {
      ...
      // eslint-disable-next-line max-len
      logger.success(`Copied '${packageDir}/dist/*' to '${nice2Folder}/${nice2Module}/resources/resources/webapp/node_modules/tocco-${packageName}/dist'`)
    }
  )
}
