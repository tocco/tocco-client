...
const setTargetEntity = (state, {payload: {entityKey}}) => {
  ...
  for (const [key, obj] of Object.entries(state.sourceData.entities.find(e => e.key === entityKey).paths)) {
  ...
  state.sourceData.relations.filter(e => e.entityKey === entityKey && e.keys.length > 0).map(e => e.relationName)
    .forEach(relationName => {
      multipleAll[relationName] = [entityKey]
    })
