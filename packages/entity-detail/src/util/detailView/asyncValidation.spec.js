      ...
      describe('asyncValidation', () => {
        describe('submitValidate', () => {
          test('should not throw an error if valid', done => {
            ...
            const values = {...mockData.baseFormValues, firstname: ''}
          ...
          test('should throw a SubmissionError', done => {
            ...
            const formValues = {...mockData.baseFormValues, firstname: ''}
            asyncValidation.submitValidate(formValues, mockData.initialValues, mockData.entityModel, mockData.mode)
              .catch(err => {
        ...
        describe('asyncValidate', () => {
          ...
          test('should not throw a request error if other errors exist', done => {
            ...
            const invalidValues = {...mockData.baseFormValues, firstname: 'illegal', phone_company: '1234'}

            asyncValidation.asyncValidate(
              invalidValues, requestMockData.initialValues, requestMockData.entityModel, requestMockData.mode)
              .catch(error => {
                expect(error).to.be.instanceof(asyncValidation.AsyncValidationException)
