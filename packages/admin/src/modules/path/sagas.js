...
export function* entityBrowserRouteHandler(location) {
  const keys = []
...
export function* locationChanged({payload}) {
  ...
  const parts = location.split('/').filter(e => e)
  if (parts.length > 0) {
    const mainRoute = parts[0]
    if (mainRoute === 'e') {
      yield call(entityBrowserRouteHandler, location)
    }
  }
}
...
export default function* mainSagas() {
  yield all([
    fork(takeLatest, actions.LOCATION_CHANGED, locationChanged)
  ])
}
