...
const Table = props => {
  ...
  const getLabel = c => {
    return c.label ? c.label : c.value
  }
...
Table.propTypes = {
   ...
   * will be referenced on each record as well as a optional order number. Lower numbers are getting displayed first.
   */
   ...
   * A records should consist of attributes that contains a value and a type attribute.
   */
