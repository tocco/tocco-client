...
describe('tocco-ui', () => {
  describe('Button', () => {
    ...
    describe('RouterLinkButton', () => {
      test('should show label', () => {
        const wrapper = enzymeUtil.mountEmbedded(<MemoryRouter><RouterLinkButton label="test"/></MemoryRouter>)
      ...
      test('should show children if no label is provided', () => {
        ...
        const wrapper = enzymeUtil.mountEmbedded(
          <MemoryRouter><RouterLinkButton>{child}</RouterLinkButton></MemoryRouter>)
        expect(wrapper.find('a').text()).to.equal(child)
      })

      test('should show icon', () => {
        const wrapper = enzymeUtil.mountEmbedded(<MemoryRouter><RouterLinkButton icon="icon"/></MemoryRouter>)
        expect(wrapper.find(Icon)).to.have.length(1)
      })
    })
  })
