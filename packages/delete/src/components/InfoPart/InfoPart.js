...
const InfoPart = ({rootEntities, relatedEntities, maxCountLink, navigationStrategy}) => (
  <Typography.Span>
    <Typography.B>
      {Object.entries(rootEntities)
        .map(([rootEntityName, rootEntity]) => (
          <span key={`root-entity-${rootEntityName}`}>
            {rootEntity.entityLabel} (
            {rootEntity.keys.length > 0 && navigationStrategy.ListLink ? (
              <navigationStrategy.ListLink entityName={rootEntityName} entityKeys={rootEntity.keys}>
      ...
      <>
        ...
        {Object.keys(relatedEntities)
          .map(entityName => {
            ...
            const Content = (
              <LinkPopOver relatedEntity={relatedEntity} maxCountLink={maxCountLink}>
                {Count}
              </LinkPopOver>
            )

            return (
              <React.Fragment key={'entity-info-' + entityName}>
                <NonBreakingText>
