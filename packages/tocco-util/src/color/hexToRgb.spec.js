...
describe('tocco-util', () => {
  describe('color', () => {
    describe('hexToRgb', () => {
      test('should convert a color with r, g and b value', () => {
        expect(hexToRgb(color2Hex)).to.eql(color2Rgb)
      })

      test('should handle short form of colors', () => {
        expect(hexToRgb(shortBlack)).to.eql(blackRgb)
      })

      test('should handle absent hashtag', () => {
        expect(hexToRgb(color2Hex.replace('#', ''))).to.eql(color2Rgb)
      })

      test('should return fallback if no valid hex is given', () => {
        expect(hexToRgb('noValidHex', color2Rgb)).to.eql(color2Rgb)
      })

      test('should return fallback if undefined hex color is given', () => {
