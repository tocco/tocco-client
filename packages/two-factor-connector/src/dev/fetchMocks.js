...
export default function setupFetchMock(packageName, fetchMock) {
  fetchMock.get(new RegExp('^.*?/nice2/rest/principals/.*/two-factor'), () => ({
    secret: '7ad4b588f0774cf19ac518289c751486',
    totpUri: 'otpauth://totp/Tocco?secret=7ad4b588f0774cf19ac518289c751486'
  }))

  fetchMock.post(new RegExp('^.*?/nice2/rest/principals/.*/two-factor'), (url, opts) =>
    JSON.parse(opts.body).userCode === '666666' ? 400 : 204
  )
  ...
  fetchMock.post(new RegExp('^.*?/nice2/rest/entities/2.0/Principal.*'), () => ({
    data: [
      {
        _links: null,
        key: '22',
