...
describe('tocco-ui', () => {
  describe('StatedValue', () => {
    ...
    test('condition error should overrule condition dirty', () => {
      const wrapper = mount(<StatedValue dirty={true} error={{error: ['error']}} touched={true} label="Label" />)
      expect(wrapper.find(StyledStatedValueBox).prop('signal')).to.be.equal('danger')
      expect(wrapper.find(StyledStatedValueLabel).prop('signal')).to.be.equal('danger')
    })
    ...
    test('should list errors', () => {
      const wrapper = mount(
        <StatedValue dirty={true} error={{error1: ['error 1-1', 'error 1-2'], error2: ['error 2']}} touched={true} />
      )
    ...
    test('should add alt attribute to label', () => {
      const wrapper1 = mount(<StatedValue label="firstname" mandatoryTitle="input is required" />)
      const el1 = wrapper1.find(StyledStatedValueLabel)
      expect(el1).to.have.length(1)
      expect(el1.prop('title')).to.be.equal('firstname')

      const wrapper2 = mount(<StatedValue label="lastname" mandatory={true} mandatoryTitle="input is required" />)
