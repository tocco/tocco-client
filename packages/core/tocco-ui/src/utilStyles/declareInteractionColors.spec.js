import {declareInteractionColors, generateInteractionColors, generateShades, shadeColor} from '../utilStyles'
...
describe('tocco-ui', () => {
  describe('utilStyles', () => {
    describe('declareInteractionColors', () => {
      test('should declare color and background for default state, :focus, :hover and :active', () => {
        const css = declareInteractionColors({
          bg: ['#000', '#030', '#003'],
          fg: ['#300', '#330', '#033']
        })
        expect(css).to.match(/background-color: #000;[\s]*color: #300;/)
        expect(css).to.match(/&:focus,[\s]*&:hover {[\s]*background-color: #030;[\s]*color: #330;[\s]*}/) // eslint-disable-line
        expect(css).to.match(
          /&:active,[\s]*&\[aria-pressed="true"\] {[\s]*background-color: #003;[\s]*color: #033;[\s]*}/
        ) // eslint-disable-line
        expect(css).to.match(/&:disabled {[\s]*background-color: #7f7f7f;[\s]*color: #997f7f;[\s]*}/) // eslint-disable-line
      })
