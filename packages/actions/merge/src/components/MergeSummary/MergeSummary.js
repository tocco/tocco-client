...
const MergeSummary = ({mergeResponse, navigationStrategy, close}) => {
  const mapToEntityList = entities =>
    Object.entries(_groupBy(entities, e => e.entityLabel))
      ...
      <StyledResultsWrapper>
        ...
        {mergeResponse.notCopiedRelations.length > 0 && (
          <StyledMessageWrapper>
            ...
            <Typography.B>{notCopiedRelations}</Typography.B>
          </StyledMessageWrapper>
        )}
        {mergeResponse.notDeletedEntities.length > 0 && (
          <StyledMessageWrapper>
            ...
            <Typography.B>{notDeletedEntities}</Typography.B>
          </StyledMessageWrapper>
        )}
